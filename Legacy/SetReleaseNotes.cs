    ...
    public sealed class SetReleaseNotes : RemoteActionBase
    {
        ...
        protected override void Execute()
        {
            ...
            if (this.IncludeReleaseNotes)
            {
                ...
                var releaseNotesTable = DB.Releases_GetReleaseNotes(this.Context.ApplicationId, this.Context.ReleaseNumber);
            ...
            if (this.IncludeIssues)
            {
                ...
                if (application.IssueTracking_Provider_Id != null)
                {
                    try
                    {
                        using (var proxy = Util.Providers.CreateProviderFromId<IssueTrackingProviderBase>(application.IssueTracking_Provider_Id.Value))
                        {
                            var categoryFilterable = proxy as ICategoryFilterable;
        ...
        protected override string ProcessRemoteCommand(string name, string[] args)
        {
            ...
            var nuspecFile = PathEx.Combine(this.Context.SourceDirectory, this.NuspecFileName);
            if (!FileEx.Exists(nuspecFile))
            {
