        ...
        protected override void Execute()
        {
            using (var agent = (IFileOperationsExecuter)Util.Agents.CreateAgentFromId(this.ServerId))
            {
                ...
                if (configurer.UseProGetClient)
                {
                    ...
                    var cmdLine = "-SolutionDirectory \"" + this.RemoteConfiguration.SourceDirectory + "\" -Source \"" + configurer.PackageSource + "\"";
                ...
                else
                {
                    ...
                    if (!configFiles.Any())
                    {
                        this.LogWarning("No packages.config files were found in {0} or any of its subdirectories.", this.RemoteConfiguration.SourceDirectory);
                        return;
                    }
                    ...
                    if (string.IsNullOrEmpty(this.PackageOutputDirectory))
                    {
                        ...
                        {
                            this.LogWarning("No .sln files found in {0} or any subdirectories.", this.RemoteConfiguration.SourceDirectory);
                            bestGuess = agent.CombinePath(this.RemoteConfiguration.SourceDirectory, "packages");
                        }
                        ...
                        {
                            this.LogWarning("Multiple .sln files were found in {0}. Using {1} to determine package install path.", this.RemoteConfiguration.SourceDirectory, solutionFiles[0].Path);
                            bestGuess = agent.CombinePath(Path.GetDirectoryName(solutionFiles[0].Path), "packages");
                        }
                    ...
                    {
                        var outputDirectory = agent.GetWorkingDirectory(this.Context.ApplicationId, this.Context.DeployableId, this.PackageOutputDirectory);
                        this.LogInformation("Packages will be installed to {0}", outputDirectory);
                        cmdLine += outputDirectory + "\"";
                    }

                    var remote = (IRemoteProcessExecuter)agent;
                    ...
                    foreach (var configFile in configFiles)
                    {
                        ...
                        this.NuGet(remote, "install", string.Format(cmdLine, configFile.Path));
                    }
                }
            }
        }
